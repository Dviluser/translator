    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            ...
            case 'delete_opinion':
                if (isset($_POST['opinion_id']) && is_numeric($_POST['opinion_id'])) {
                    $opinion_id = intval($_POST['opinion_id']);
                    $sql = "DELETE FROM opinions WHERE id = $opinion_id";
                    
                    if ($conn->query($sql)) {
                        $_SESSION['success_message'] = 'Opinion deleted successfully!';
                    } else {
                        $_SESSION['error_message'] = 'Error deleting opinion: ' . $conn->error;
                    }
                }
                break;
...
if (!empty($members_search)) {
    $members_where = " WHERE name LIKE '%$members_search%' OR email LIKE '%$members_search%' OR phone LIKE '%$members_search%' OR address LIKE '%$members_search%'";
}
...
if (!empty($opinions_search)) {
    $opinions_where = " WHERE name LIKE '%$opinions_search%' OR email LIKE '%$opinions_search%' OR phone LIKE '%$opinions_search%' OR opinion LIKE '%$opinions_search%'";
}
...
<head>
    ...
    <style>
        /* Additional styles for address column */
        .address-cell {
            max-width: 200px;
            white-space: nowrap;
            overflow: hidden;
        ...
        <div class="content-section fade-in">
            ...
            <div class="search-container">
                <form method="GET" action="dashboard.php" class="search-box">
                    ...
                    <input type="text" class="search-input" name="members_search" placeholder="Search members by name, email, phone or address..." value="<?php echo isset($_GET['members_search']) ? htmlspecialchars($_GET['members_search']) : ''; ?>">
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="address-cell" title="<?php echo htmlspecialchars($member['address'] ?? 'No address provided'); ?>">
                                <?php 
                                if (!empty($member['address'])) {
                                    echo htmlspecialchars(substr($member['address'], 0, 30)) . (strlen($member['address']) > 30 ? '...' : '');
                                } else {
                            ...
                            <td>
                                <form method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete this member?');">
                                    ...
                                    <button type="submit" class="action-btn btn-delete" style="background-color: #dc3545;">
        ...
        <div class="content-section fade-in">
            ...
            <div class="search-container">
                <form method="GET" action="dashboard.php" class="search-box">
                    ...
                    <input type="text" class="search-input" name="opinions_search" placeholder="Search opinions by name, email, phone or content..." value="<?php echo isset($_GET['opinions_search']) ? htmlspecialchars($_GET['opinions_search']) : ''; ?>">
                    <button type="submit" class="search-btn"><i class="fas fa-search"></i> Search</button>
    ...
    <script>
        function viewOpinion(opinionId) {
                    ...
                    modalBody.innerHTML = `
                        <div class="opinion-detail">
                            <div class="detail-row">
                                <strong>Name:</strong> <span>${opinion.name}</span>
                            </div>
                            <div class="detail-row">
                                <strong>Email:</strong> <span>${opinion.email}</span>
                            </div>
                            <div class="detail-row">
                                <strong>Phone:</strong> <span>${opinion.phone}</span>
                            </div>
                            <div class="detail-row">
                                <strong>Language:</strong> <span>${opinion.language}</span>
                            </div>
                            <div class="detail-row">
                                <strong>Submitted on:</strong> <span>${opinion.submission_date}</span>
                            </div>
                            <div class="detail-row">
                                <strong>Status:</strong> <span>${opinion.status}</span>
                            </div>
                            <div class="detail-row">
            ...
            .catch(error => {
                ...
                alert('An error occurred while fetching the opinion');
            });
        }
    </script>
